...
import { login } from '@api/setting/account'; 
            ...
            return new Promise((resolve, reject) => {
                ...
                login({
                    ...
                    .then(async res => {
						...
						if(loginInfo.info.avatar!==''){
							loginInfo.info.avatar = Setting.ossURL+loginInfo.info.avatar
						}
                        ...
                        util.cookies.set('uuid', loginInfo.uuid);
                        util.cookies.set('token', loginInfo.token);
						// util.cookies.set('proj', res.data.project_info);
                        // 设置 vuex 用户信息
						
						const auth = {
								menu_list:loginInfo.menu_list,
								is_admin:loginInfo.is_admin
						};
						const userInfo = Object.assign(loginInfo.info, auth);
						console.log("1212");
						console.log(userInfo);
						console.log("1212");
                        await dispatch('admin/user/set', loginInfo.info, { root: true });
						await dispatch('admin/user/setProjectInfo', loginInfo.project_info, { root: true });
                        // 用户登录后从持久化数据加载一系列的设置
                        await dispatch('load');
                        // 结束
                        resolve(res);
