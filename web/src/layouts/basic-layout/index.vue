<template>
    <Layout class="i-layout">
        <Sider v-if="!isMobile && !hideSider" class="i-layout-sider" :class="siderClasses" :width="menuSideWidth">
            <i-menu-side :hide-logo="isHeaderStick && headerFix && showHeader" :currentProj="currentProj" @on-project="handleProject" />
        </Sider>
	...
	<!--	<projectSelect :currentProj="currentProj" @on-proj-select="handleProjSelect" v-if="projectShowStatus" v-model="projectShow" @add-closed="handleProjClose"></projectSelect> -->
    </Layout>
...
<script>
	...
	import iHeaderCompany from './header-company';
    ...
    export default {
        name: 'BasicLayout',
        components: { iMenuHead, iMenuSide, iCopyright, iHeaderLogo, iHeaderCollapse, iHeaderReload, iHeaderBreadcrumb, iHeaderSearch, iHeaderCompany,iHeaderUser, iHeaderI18n, iHeaderLog, iHeaderFullscreen, iHeaderSetting, iHeaderNotice, iTabs,projectSelect },
        data () {
            return {
                showDrawer: false,
                ticking: false,
				...
				currentProj:{proj_name:'未选择',id:0}
            }
        },
        computed: {
        ...
        methods: {
			...
			handleProject(){
				this.projectShowStatus = true;
				this.$nextTick(() => {
					this.projectShow = true;
				});
			},
			handleProjSelect(val){
				
				const project_info = {id:val.id,proj_name:val.proj_name };
				this.$store.dispatch('admin/user/setProjectInfo', project_info, { root: true });
				this.currentProj = project_info;
			},
			handleProjClose(){
				this.handleReload();
				this.projectShowStatus = false
				this.$nextTick(() => {
